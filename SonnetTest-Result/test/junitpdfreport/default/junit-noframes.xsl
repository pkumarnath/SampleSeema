...
<fo:block font-size="10pt" font-family="Times">
    ...
    <fo:table border-collapse="separate" inline-progression-dimension.maximum="60%" inline-progression-dimension="17.5cm" table-layout="fixed" background-color="lightgray" space-before="0.1cm">
  ...
  <fo:block font-size="10pt" font-family="Times" inline-progression-dimension.maximum="60%">
    ...
    <fo:table border-collapse="separate" inline-progression-dimension.maximum="60%" inline-progression-dimension="17.5cm" table-layout="fixed" background-color="lightgray" space-before="0.5cm">
      <fo:table-column column-width="6.5cm"/>
      <fo:table-column column-width="3cm"/>
      <fo:table-column column-width="3cm"/>
      <fo:table-column column-width="3cm"/>
...
<xsl:template name="testsuite.packagelist.line">
  ...
  <xsl:template name="packages">
    <xsl:for-each select="/testsuites/testsuite[not(./@package = preceding-sibling::testsuite/@package)]">
      <xsl:sort select="@package"/>
        <fo:block space-before.optimum="15pt" space-after.optimum="0pt" font-size="11pt">
          Test Suite: <xsl:value-of select="@package"/>
        </fo:block>
...
<xsl:template name="classes">
  <xsl:for-each select="testsuite">
    <xsl:sort select="@name"/>
    			  <xsl:variable name="methodtestCount" select="@tests"/>
				  <xsl:variable name="methoderrorCount" select="@errors"/>
				  <xsl:variable name="methodfailureCount" select="@failures"/>
				  <xsl:variable name="methodsuccessCount" select="($methodtestCount - $methoderrorCount - $methodfailureCount)"/>
    ...
    <fo:page-sequence master-reference="simpleA4">
      ...
      <fo:flow flow-name="xsl-region-body">
		...
		<fo:block space-before.optimum="8pt" space-after.optimum="5pt">
		    
       
      
        </fo:block>
			
        <fo:block space-before.optimum="8pt" space-after.optimum="0pt" >
          Test Suite:<xsl:value-of select="@package"/> <fo:leader leader-pattern="space" /> Test Script:<xsl:value-of select="@name"/>
        </fo:block>
...
<xsl:template name="page-header">
  <fo:block font-size="20pt" text-align="start" font-weight="bold"
            border-style="solid" border-color="blue" border-width="0pt">
    <fo:table border-collapse="separate" inline-progression-dimension.maximum="100%" inline-progression-dimension="18cm" table-layout="fixed">
        ...
        <fo:table-row>
          ...
          <fo:table-cell >
            <fo:block text-align="right">
              ...
              <fo:external-graphic src="{$Product_Logo}" content-height="scale-to-fit" height="0.4in"  content-width="1.0in" scaling="non-uniform" text-align="right"/>
  ...
  <fo:block><fo:leader leader-length="100%" leader-pattern="rule"  rule-thickness="1px"/></fo:block>
